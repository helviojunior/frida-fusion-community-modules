...
class AntiRoot(ModuleBase):

    def __init__(self):
        super().__init__('fridantiroot', 'Use fridantiroot by @dzonerzy')
        self.mod_path = str(Path(__file__).resolve().parent)
        self.js_file = os.path.join(self.mod_path, "fridantiroot.js")

    def start_module(self, **kwargs) -> bool:
        try:
            if not os.path.isfile(self.js_file):
                Logger.pl("{+} Downloading CodeShare script from @dzonerzy/fridantiroot")
                data = self._get_codeshare("@dzonerzy/fridantiroot")
